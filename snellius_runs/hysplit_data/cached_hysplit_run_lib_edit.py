...
hysplitPATH = "/home/nholzbach/hysplit/"
PATH ="/home/nholzbach/hysplit_data"
dispPATH = "/earthtime/air-src/dispersionCache/"
dispSitltPATH = "/earthtime/air-data/dispersionStiltCache/"
hrrrPATH = "/earthtime/air-data/hrrr/"
...
class DispersionSource:
    ...
    def __init__(self, name, ID, lat, lon, minHeight, maxHeight, emit_file, areaSqM=0):
        ...
        # assert if nothing entered for emit_file
        assert(emit_file != None)
        self.emit_scheme = pd.read_csv(emit_file).set_index('Hour')
...
#assert(DispersionSource(name='Test',ID=0, lat=40.123456789, lon=-79.123456789,
#    minHeight=10, maxHeight=50, emit_file='emit_test.csv').cachePath() == '40.123457,-79.123457_10-50')
#assert(DispersionSource(name='Test',ID=0, lat=40.123456789, lon=-79.123456789,
#    minHeight=10, maxHeight=50, areaSqM=100, emit_file='emit_test.csv').cachePath() == '40.123457,-79.123457_10-50_100')

# assert(DispersionSource(name='Test', lat=40.123456789, lon=-79.123456789,
#     minHeight=10, maxHeight=50).cachePath() == 'Test')
# assert(DispersionSource(name='Test', lat=40.123456789, lon=-79.123456789,
#     minHeight=10, maxHeight=50, areaSqM=100).cachePath() == 'Test_100')

class CachedDispersionRun:
    """
    Input:
        source -- DispersionSource representing lat/lon and altitude min/max, and emission scheme of emission source
    ...
    def __init__(self, source, runStartLocal, emitTimeHrs, runTimeHrs, hysplitModelSettings,
                 fileName='cdump', hysplit_root=hysplitPATH, verbose=False,
                 dispersionCachePath='/air-data/dispersionCache',
                 hrrrDirPath='/air-data/hrrr',
                 useForecast=False):
        try:
    ...
    def run(self):
        ...
        with lock:
            ...
            try:
                ...
                # self.interpolate(self.cdumpPath(), self.interpPath(), self.stationPath())
                # self.readInterpFile()
                # self.saveToText()
    ...
    # def cdumptextPath(self):
    #     return self.path() + '/cdumptext'

    # def shortCdumptextPath(self):
    #     return self.shortPath() + '/cdumptext'

    def interpPath(self):
        return self.path() + '/interp'

    # don't need this rn but might be useful later
...
{#          #}{{ run.source.lat }} {{ run.source.lon }} {{ height }} 0.0 {{ run.source.areaSqM }} #3: SOURCE LATITUDE | LONGITUDE | HEIGHT(m-agl)
{#          #}{%- endfor -%}
...
{#          #}{{run.source.emit_scheme['emission'][run.runHr]}} #12: EMISSION RATE (per hour)
...
def getDispersionRun(source,runStartLocal,emitTimeHrs,runTimeHrs,hysplitModelSettings,verbose=False,
        dispersionCachePath=PATH+dispPATH,
        hrrrDirPath=PATH+hrrrPATH):
...
def getMultiHourDispersionRunsParallel(source,runStartLocal,emitTimeHrs,totalRunTimeHrs,
        ...
        dispersionCachePath=PATH+dispSitltPATH,
        hrrrDirPath=PATH+hrrrPATH ,useForecast=False):
