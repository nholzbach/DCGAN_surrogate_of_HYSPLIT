...
# this restructures the pardump file in order to generate images. This is what we DON'T want
def parse_pardump(fname, filter_ratio=0.8, with_size=False):
...
def create_bin(fnames, o_file, colormap):
    ...
    for fname in fnames:
        ...
        points += parse_pardump(fname)
        lines = []
        i += 1
    array.array('f', points).tofile(open(o_file, 'wb'))
    points = []

# this is what gets used in other files, 
def create_multisource_bin(fnames, o_file, numSources, filter_out_ratios=0.8):
    """
    ...
    for i in range(numSources):
        ...
        pool.submit(particle_dat_to_bin,single_source,filter_out)
...
def particle_dat_to_bin(filenames,filter_out):
