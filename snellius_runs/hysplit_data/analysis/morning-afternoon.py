...
def get_impact_stats(df, date, plot=False, title=None, plotsave=False):
 if df is None:
    ...
    if plot == True:
        ...
        # plt.title(f'Impact analysis for {date}, {title}')
        if title == 'avg':
...
def main(argv):
  ...
  for day in range(1,32):
    ...
    if os.path.exists(hysplit_file):
        ...
        # do for each hour range
        ranges = [range(6, 14 + 1), range(14, 22 + 1)]
        name = ['morning', 'afternoon']
        for name, hour_range in zip(name, ranges):
            time = f'{yr}-{month}-{day}-{name}'
            print(time)
            # hour_range = range(6, 14 + 1)  # +1 to include the end hour
            hysplit_morning_result = hysplit_day_result[hysplit_day_result['HR'].isin(hour_range)]
            hysplit_result_gdf = preprocess(hysplit_morning_result, bbox, crs)
            ...
            group_df = smell_reports.loc[(smell_reports['Year'] == int(yr)) & (smell_reports['Month'] == int(month)) & (smell_reports['Day'] == day) & (smell_reports['Hour'].isin(hour_range))]
            if len(group_df) == 0:
                print('no reports for this hour')
                continue
            if len(hysplit_merged) == 0:
                print("no hysplit I guess?")
            else:
                ...
                if (df_avg is not None) or (df_sum is not None):
                    impact_stats_avg, corr_avg = get_impact_stats(df_avg, time, plot=True,title='avg', plotsave=True)
                    impact_stats_sum, corr_sum = get_impact_stats(df_sum, time, plot=True,title="sum", plotsave=True)
  ...
  stats_df.to_csv(f'CR_comparison/{yr}-{month}_morningafternoon_results.csv')
